...
{
    public static class ToolsCalender
    {
        ...
        public static string ToFarsiMonth(this DateTime date)
        {
            if (date == new DateTime()) return "";
            var pc = new PersianCalendar();
            var i =Convert.ToInt32($"{pc.GetMonth(date):00}");            
            return MonthNames[i-1];
        }
         public static string ToFarsiDay(this DateTime date)
        {
            if (date == new DateTime()) return "";
            var pc = new PersianCalendar();
            return $"{pc.GetDayOfMonth(date):00}";
        }
